    ...
    function get_setting_limitation(string $package): array
    {
        $data = [
            'free'     => [
                ...
                'images'           => 1,
                'branch_master'    => 1,
                'timeslot_booking' => true,
                'session_booking'  => false,
...
if (!function_exists('get_country_list')) {
    function get_country_list($country_code = ''): array|string|null
    {
        $codes = [
            ...
            "ER" => "إرتريا (Eritrea)",
            "JO" => "الأردن (Jordan)",
            "AE" => "الإمارات العربيّة المتّحدة (United Arab Emirates)",
            "BH" => "البحرين (Bahrain)",
            "DZ" => "الجزائر (Algeria)",
...
if (!function_exists('get_country_subdivisions')) {
    function get_country_subdivisions($country_code, $subdivision_code = ''): array|string|null
    {
        $codes   = [
            "TH" => [
                "en" => [
        ...
        if (isset($codes[$country_code])) {
            ...
            if (!empty($subdivision_code)) {
                return $codes[$country_code][$lang][$subdivision_code] ?? @$codes[$country_code]['en'][$subdivision_code];
            }
            return $codes[$country_code][$lang] ?? @$codes[$country_code]['en'];
        }
        // country is not supported now
        return null;
    }
}
